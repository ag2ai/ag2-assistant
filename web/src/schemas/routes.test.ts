...
// Generated by `npm test`'s pretest hook (scripts/gen_openapi.sh), never committed:
// nothing ships this document, so a copy in git would only guarantee itself.
const SPEC = new URL('../../.openapi.json', import.meta.url)
if (!existsSync(SPEC)) {
  throw new Error(
    'web/.openapi.json is missing — generate it with `python3 scripts/dump_openapi.py`, ' +
      'or run `npm test`, which does that for you.',
  )
}
