...
import { closeOverlay, openOverlay, route } from '../../router.ts'
import { errText } from '../../lib/errors.ts'
import type { ProfileSettings } from '../../schemas/settings.ts'
import type { GoogleStatus } from '../../schemas/system.ts'
import {
...
export type SettingsContext = {
  s: ProfileSettings | null   // GET /api/settings payload (null until load() resolves)
  google: GoogleStatus | null // GET /api/google/status
  drafts: Record<string, string> // provider -> input value (API-key drafts)
  busy: boolean
  err: string
  load: () => Promise<void>
  run: (fn: () => Promise<unknown>) => Promise<void>
  close: () => void
  openVoice: (configId?: string | null) => void
  openGoogle: () => void
  openCodex: () => void
  openPoweredBy: () => void
  reRunSetup: () => void
...
export function createSettingsContext(): SettingsContext {
  const ctx: SettingsContext = $state({
    s: null,
    google: null,
    drafts: {},
    busy: false,
    ...
    run: async (fn) => {
      ...
      try { await fn(); await ctx.load() } catch (e) { ctx.err = errText(e) }
      ctx.busy = false
    },
    ...
    openVoice: (configId = null) => { voicePickerConfig.set(configId); voicePickerOpen.set(true) },
    openGoogle: () => { closeOverlay(); googleOpen.set(true) },
    ...
    openPoweredBy: () => openOverlay('poweredby', get(route).overlayValue),
    reRunSetup: () => { closeOverlay(); onboardingOpen.set(true) },
  })
