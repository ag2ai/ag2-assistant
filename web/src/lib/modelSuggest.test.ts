...
import {
  ...
  parseCatalogPayload,
  probeStatusReason,
} from './modelSuggest.ts'
import type { ModelSuggestion, ProbeRequest } from './modelSuggest.ts'
import { KNOWN_MODELS, knownModel } from './knownModels.ts'

const ids = (rows: ModelSuggestion[]) => rows.map((r) => r.id)

// Every assertion below is about a request that must be built, so resolve-or-fail
// once rather than null-guarding in each test.
function probe(target: Parameters<typeof browserProbeRequest>[0]): ProbeRequest {
  const req = browserProbeRequest(target)
  assert.ok(req, 'no browser request was built')
  return req
}
...
test('a pasted key with URL-hostile characters still reaches Gemini intact', () => {
  const req = probe({ type: 'gemini', key: 'a+b/c=d&e' })
...
test('Anthropic needs its own header, plus permission to be called from a browser', () => {
  const req = probe({ type: 'anthropic', key: 'sk-ant' })
...
test('a custom endpoint is asked at its own address, not the vendor’s', () => {
  const req = probe({ type: 'openai', baseUrl: 'http://localhost:8080/v1/', key: 'k' })
