...
export const refLabel = (name) => '@' + String(name ?? '')
...
export function applyPick(text, start, caret, label) {
  const t = String(text ?? '')
...
// along, in pick order.
export function reconcile(picks, text) {
  const t = String(text ?? '')
  const budget = new Map()
  const out = []
  for (const p of picks || []) {
    if (!budget.has(p.label)) budget.set(p.label, countTokens(t, p.label))
    if (budget.get(p.label) > 0) {
      budget.set(p.label, budget.get(p.label) - 1)
      out.push(p)
    }
  }
  return out
...
export function buildBlock(picks) {
...
export function highlightSegments(text, picks) {
  const t = String(text ?? '')
...
export function composeMessage(text, picks) {
