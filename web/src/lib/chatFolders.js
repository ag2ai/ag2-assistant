...
export const taskGrant = (f, pid, taskId) =>
  taskId ? (f.grants || []).find((g) => g.profile === pid && g.task_id === taskId && !g.chat_id) : null
const hasProfileGrant = (f, pid) => (f.grants || []).some((g) => g.profile === pid && !g.chat_id && !g.task_id)
...
const inheritedGrant = (f, pid, taskId) => {
...
// instead.
export function chatChips(folders, pid, chatId, taskId = '') {
  return (folders || [])
    .map((f) => {
...
export function inheritedCount(folders, pid, chatId, taskId = '') {
...
export function addPlan(folder, pid, chatId, taskId = '') {
