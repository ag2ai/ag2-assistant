...
import { API_INTERFACES, usesBaseUrl, offersApiInterface, settleWithoutBaseUrl } from './apiInterface.js'
import { TYPE_LABEL, TYPE_GROUP } from './providerLabels.js'
...
test('naming an endpoint reveals the choice — and only for the endpoint-addressable types', () => {
  for (const type of ALL_TYPES) {
    ...
    assert.equal(shown, API_INTERFACES.includes(type), `${type} is offered the wrong way round`)
  }
})
...
test('the types that take a Base URL are the types that can choose a wire', () => {
  for (const type of ALL_TYPES) {
    assert.equal(usesBaseUrl(type), API_INTERFACES.includes(type), `${type} shows the wrong endpoint field`)
