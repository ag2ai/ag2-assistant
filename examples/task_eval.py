...
# --- SAFETY: keep Drive + Calendar (read-only), remove Gmail + writes ---
...
async def run_one(cfg, scn: dict) -> dict:
    ...
    try:
        await prepare_task(store, t.id, planner_agent, user)
    ...
    result = {"name": scn["name"], "status": status, "elapsed": elapsed,
              ...
              "subtasks": [(c.title, c.status) for c in await store.children(t.id)]}
