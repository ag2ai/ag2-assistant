"""Paired accounts — who may reach a Channel at all (ADR 0021).
...
account presenting it, and is matched by id ever after.
"""
...
def test_a_handle_stays_pending_until_someone_bearing_it_speaks():
    account = pairing.add_account("telegram", "@nikita")
...
def test_a_handle_is_entered_with_or_without_its_at_sign():
    assert pairing.add_account("telegram", "nikita").handle == "nikita"
...
def test_a_handle_is_refused_where_messages_carry_none():
    ...
    try:
        pairing.add_account("slack", "@nikita")
    except ValueError as exc:
        assert "numeric" in str(exc)
    else:
        raise AssertionError("a handle should not be accepted for slack")
    assert pairing.add_account("slack", "42").account_id == "42"


def test_an_account_is_paired_on_one_platform_only():
    pairing.add_account("telegram", "42")
...
def test_entering_the_same_account_twice_leaves_one_entry():
    pairing.add_account("telegram", "42")
    pairing.add_account("telegram", "42")
    assert len(pairing.list_accounts("telegram")) == 1
...
def test_the_first_account_bearing_a_pending_handle_pins_it():
    ...
    assert pairing.is_paired("telegram", "42", "nikita") is True

    pinned = pairing.list_accounts("telegram")[0]
...
def test_after_pinning_a_later_holder_of_the_handle_is_not_admitted():
    ...
    pairing.add_account("telegram", "@nikita")
    pairing.is_paired("telegram", "42", "nikita")
    assert pairing.is_paired("telegram", "99", "nikita") is False


def test_after_pinning_the_original_account_keeps_access_under_a_new_handle():
    pairing.add_account("telegram", "@nikita")
    pairing.is_paired("telegram", "42", "nikita")
    assert pairing.is_paired("telegram", "42", "someone-else-entirely") is True
    assert pairing.is_paired("telegram", "42", None) is True
...
def test_a_code_pairs_the_account_that_presents_it():
    ...
    assert pairing.redeem("telegram", code, "42") == pairing.PAIRED
    assert pairing.is_paired("telegram", "42") is True


def test_a_code_records_the_handle_of_the_account_that_used_it():
    code = pairing.issue_code("telegram")
    pairing.redeem("telegram", code, "42", "@nikita")
    assert pairing.list_accounts("telegram")[0].handle == "nikita"


def test_a_code_cannot_be_used_twice():
    code = pairing.issue_code("telegram")
    pairing.redeem("telegram", code, "42")
    assert pairing.redeem("telegram", code, "99") == pairing.UNKNOWN
...
def test_an_expired_code_is_refused_and_reported_as_expired():
    ...
    assert pairing.redeem("telegram", code, "42") == pairing.EXPIRED
    assert pairing.is_paired("telegram", "42") is False


def test_a_code_is_read_whatever_case_it_is_typed_in():
    code = pairing.issue_code("telegram")
    assert pairing.redeem("telegram", code.lower(), "42") == pairing.PAIRED


def test_a_code_issued_for_one_channel_does_not_pair_on_another():
    code = pairing.issue_code("telegram")
    assert pairing.redeem("discord", code, "42") == pairing.UNKNOWN


def test_an_unissued_code_is_simply_unknown():
    assert pairing.redeem("telegram", "AAAA-1111", "42") == pairing.UNKNOWN
...
def test_the_live_code_is_visible_in_settings_until_it_is_used():
    code = pairing.issue_code("telegram")
    assert pairing.live_code("telegram").code == code
    pairing.redeem("telegram", code, "42")
    assert pairing.live_code("telegram") is None
...
def test_issuing_a_second_code_replaces_the_first():
    ...
    first = pairing.issue_code("telegram")
    second = pairing.issue_code("telegram")
    assert pairing.redeem("telegram", first, "42") == pairing.UNKNOWN
    assert pairing.redeem("telegram", second, "42") == pairing.PAIRED
...
def test_revoking_a_pinned_account_takes_effect_immediately():
    account = pairing.add_account("telegram", "42")
    assert pairing.revoke("telegram", account.key) is True
    assert pairing.is_paired("telegram", "42") is False


def test_revoking_a_pending_handle_withdraws_the_invitation():
    account = pairing.add_account("telegram", "@nikita")
    pairing.revoke("telegram", account.key)
    assert pairing.is_paired("telegram", "42", "nikita") is False


def test_revoking_something_that_is_not_there_reports_so():
    assert pairing.revoke("telegram", "42") is False


def test_revoking_one_account_leaves_the_others_alone():
    pairing.add_account("telegram", "42")
    keep = pairing.add_account("telegram", "99")
    pairing.revoke("telegram", "42")
    assert [a.key for a in pairing.list_accounts("telegram")] == [keep.key]
...
def test_the_paired_accounts_survive_a_restart():
    ...
    pairing.add_account("telegram", "42")
    assert pairing.list_accounts("telegram")[0].account_id == "42"
