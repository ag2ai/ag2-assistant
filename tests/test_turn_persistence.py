...
async def _gateway(tmp_path, monkeypatch, agent):
...
async def test_failed_turn_is_persisted(tmp_path, monkeypatch, exc):
    gw = await _gateway(tmp_path, monkeypatch, BoomAgent(exc))
    try:
...
async def test_failed_turn_survives_a_reload(tmp_path, monkeypatch):
    """The point of persisting: a fresh Gateway over the same data dir still has it."""
    gw = await _gateway(tmp_path, monkeypatch, BoomAgent(RuntimeError("boom")))
    with pytest.raises(RuntimeError):
        await gw.send_message("do the thing", chat_id="c1")
    await gw.close()

    gw2 = await _gateway(tmp_path, monkeypatch, BoomAgent(RuntimeError("boom")))
    try:
...
async def test_timeout_failure_reads_as_a_timeout(tmp_path, monkeypatch):
    gw = await _gateway(tmp_path, monkeypatch, BoomAgent(asyncio.TimeoutError()))
    try:
...
async def test_shutdown_cancellation_keeps_the_turn(tmp_path, monkeypatch):
    ...
    gw = await _gateway(tmp_path, monkeypatch, HangingAgent(running))
    try:
...
async def test_user_stop_still_marks_the_turn_cancelled(tmp_path, monkeypatch):
