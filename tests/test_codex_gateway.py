...
subscription toggle on POST /api/p/{pid}/settings/llm."""
...
def test_settings_llm_subscription_requires_sign_in(profile_app):
    client, pid = profile_app
    # Not signed in → 409 with a sign-in hint.
    r = client.post(
        f"/api/p/{pid}/settings/llm", json={"provider": "openai", "auth_mode": "subscription"}
    )
    assert r.status_code == 409
    assert "ChatGPT" in r.json()["error"]


def test_settings_llm_subscription_persists_when_signed_in(profile_app):
    client, pid = profile_app
    codex_auth._store_tokens({"access_token": "A", "refresh_token": "R", "expires_in": 3600})
    r = client.post(
    ...
    assert settings["assistant"]["auth_mode"] == "subscription"
    assert settings["codex"]["signed_in"] is True
