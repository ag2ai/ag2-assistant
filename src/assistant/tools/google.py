...
def build_google_tools(auth: GoogleAuth) -> list:
    """All Google tools, bound to one install's credentials.

    Built per call so the API clients come from the injected ``GoogleAuth`` rather
    than a module-level singleton.
    """

    def _service(api: str, version: str):
        return auth.build_service(api, version)
    ...
    async def gmail_search(
        query: Annotated[
            str, Field(description="Gmail search query, e.g. 'from:alice is:unread'.")
        ],
    ...
    async def calendar_create_event(
        ...
        start: Annotated[
            str, Field(description="ISO start datetime, e.g. 2026-06-20T14:00:00+10:00.")
        ],
    ...
    async def drive_read(
        file_id: Annotated[
            str,
            Field(description="The Drive file id (from drive_search) or a Docs/Sheets/Drive URL."),
        ],
    ) -> str:
