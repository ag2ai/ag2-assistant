...
# How long a minted code stays presentable — the spec's ten minutes.
CODE_TTL = 10 * 60.0
...
class PairingStore:
    """One install's paired accounts (``pairing.json``). Per Connection, never per
    profile: who may reach a Connection is the Connection's own business (ADR 0021)."""

    def __init__(self, paths: Paths) -> None:
        self._path = paths.root / "pairing.json"
    ...
    def list_accounts(self, connection: str) -> list[PairedAccount]:
        """Every account allowed to reach ``connection``, in the order they were added."""
        return [_account(e) for e in self._load()["accounts"] if e.get("connection") == connection]

    def add_account(self, connection: str, value: str, platform: str) -> PairedAccount:
        ...
        if value.isdigit():
            ...
            if platform not in HANDLE_PLATFORMS:
                raise ValueError(
                    f"{platform} messages carry no handle — enter a numeric account id"
                )
    ...
    def revoke(self, connection: str, key: str) -> bool:
    ...
    def is_paired(self, connection: str, account_id: str, handle: str | None = None) -> bool:
    ...
    def issue_code(self, connection: str, *, ttl: float = CODE_TTL) -> str:
        ...
        data["codes"].append(
            {"connection": connection, "code": code, "expires_at": time.time() + ttl}
        )
        self._write(data)
        return code

    def live_code(self, connection: str) -> PairingCode | None:
    ...
    def redeem(self, connection: str, code: str, account_id: str, handle: str | None = None) -> str:
        ...
        match = next(
            (
                e
                for e in data["codes"]
                if e.get("connection") == connection and e.get("code") == wanted
            ),
            None,
        )
        if match is None:
