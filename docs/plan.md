...
- **Supervisor / watchdog — persistent, deterministic (no LLM).** An always-on
  ...
  **restarts** anything that's down or wedged. Also supports **hot-reload**: when
  ...
  - *Why:* keep a long-running personal assistant up unattended; pick up agent
    updates safely. Pairs with `ag2assistant run` (it would own the channel/gateway
...
- **Onboarding — get to know the user (seeded question set).** A first-run flow that proactively asks a short set of high-value questions instead of waiting to learn them passively. Captures things the agent keeps needing: **location/timezone, working hours, name, preferred tone, channels, do-not-disturb**, etc.
  - *Mechanism:* reuse the HITL `Asker` (buttons/free-text) so onboarding runs on whichever surface started it (desktop popup, or a chat). A `ag2assistant onboard` command + auto-trigger on first interaction when the profile is empty.
  - *Storage:* answers seed the profile memory (`/memory/working.md`) and, where structured, config (e.g. write `AG2ASSISTANT_LOCATION`). The passive observer then keeps refining over time.
...
- [x] `Asker` protocol + `build_hitl_hook()` adapter (`ag2assistant/hitl/base.py`) — wires to AG2 `context.input()`/`hitl_hook`
...
- [x] `create_agent(asker=...)` wiring + `ag2assistant agent` defaults to DesktopAsker
- [x] `PermissionStore` (persistent `~/.ag2assistant/permissions.json`) — per-folder grants, ancestor coverage, Once / Always / Deny via `request_access()`
...
- [x] **`ag2assistant permissions` CLI** — `list` / `allow` / `revoke` / `block` / `unblock`. Persistent **block** (permanent deny) added to `PermissionStore` and honored by `PermissionManager.check` (never even prompts for a blocked folder).
- [x] **Onboarding (seeded questions)** — `ag2assistant onboard` command + auto-trigger on first interaction when the profile is empty. Asks name / location / working hours / answer-style through the same pluggable `Asker` (desktop popup or chat buttons/free-text), all skippable. Seeds the profile (`/memory/working.md`, preserving any existing) and writes `AG2ASSISTANT_LOCATION` to `.env`; one-time marker at `~/.ag2assistant/onboarded`. (`ag2assistant/onboarding.py`, gateway `_maybe_onboard`, 8 + 2 tests)
- [x] **Chat attachments** — files dropped into Telegram/Discord/Slack are downloaded and turned into AG2 multimodal `Input`s (`ag2assistant/attachments.py`: images→Image, PDFs/unknown→Document, audio/video→native, text→inlined) and threaded through `Gateway.send_message(attachments=...)` as positional inputs. Slack uses the bot token to fetch private file URLs (`files:read`). (8 tests; channel normalizers now accept caption-only/attachment-only messages)
- [x] **Docker sandbox for shell/code** — custom `DockerEnvironment`/`DockerSandbox` (`ag2assistant/tools/docker_sandbox.py`) implementing AG2's `Sandbox` protocol (AG2 ships no Docker backend yet). Runs shell/code in a throwaway container with **no host FS mount**, so the approval gate is dropped when active (the container *is* the boundary); `read_file` stays host-permission-gated. Falls back to local+approval with a warning if Docker is unavailable. Selectable via `--sandbox docker` or `AG2ASSISTANT_SANDBOX`/`AG2ASSISTANT_DOCKER_IMAGE`/`AG2ASSISTANT_DOCKER_NETWORK`. Live-verified end-to-end (agent computed Fibonacci in-container with no prompt; containers cleaned up). (6 unit + 2 real-Docker integration tests)
- [x] **Combined `ag2assistant run`** — one process serving the REST/WS gateway + every channel whose token is configured, all sharing one `Gateway`/agent/profile (`create_app(gateway=...)` reuse). `--no-rest`, `--port`, `--sandbox` flags. (3 tests)
- [x] **Mount HITL routes on the gateway** — the running gateway now serves the styled `/hitl/{id}` pages + `/hitl/{id}/answer` (shared `HitlServer` registry via `add_hitl_routes`), plus `GET /api/hitl/pending`. `GatewayAsker` registers questions there; REST clients poll+answer, WS clients get a pushed `question` frame and answer over the same socket (`_drive_turn` reads answer frames concurrently while the turn is blocked). Request timeout → safe deny (default 5 min). (`ag2assistant/hitl/gateway.py`, `hitl/desktop.py` refactor; 4 tests incl. full WS Q&A flow)
- [x] **Sandbox skill-script execution (Docker)** — `DockerMountSandbox` + `build_docker_skill_runtime` (`ag2assistant/tools/docker_sandbox.py`): when `--sandbox docker`, each skill script runs in a one-shot `docker run --rm` container that bind-mounts **only that skill's `scripts/` dir** (so untrusted skill code can't reach the user's files); storage/discovery stay local. Wired into `build_skills_toolkit`. (2 unit + 2 real-Docker integration tests)
- [x] **Tuned aggregation cadence** — profile distillation now batches `every_n_turns` (default 4, `AG2ASSISTANT_AGGREGATE_EVERY_N`) instead of firing `on_end` every message; single-shot `ag2assistant agent` still aggregates `on_end` so its one turn is captured (`create_agent(single_shot=...)`). (3 tests)
- [x] **Interactive `ag2assistant chat`** — multi-turn terminal REPL over a single `Gateway` session (memory, permissions popup, onboarding all apply); non-blocking input via `asyncio.to_thread`. Live-verified cross-turn recall.
...
- [x] CLI interface (`ag2assistant agent "message"` working)
...
- [x] Multi-turn interactive conversation via CLI (`ag2assistant chat`)

### Phase 5 Detail — Observer Memory (in progress)

- [x] Persistent user-profile store (`SqliteKnowledgeStore`, `~/.ag2assistant/profile.db`)
...
- [x] CLI: `ag2assistant profile show` / `ag2assistant profile clear`, `--memory/--no-memory`, `--platform`
...
- [x] `ag2assistant gateway` CLI (uvicorn)
...
- [x] Channel layer (`ag2assistant.channels`) — `Channel` ABC, `InboundMessage`, `should_respond` gating, `get_channel` factory
...
- [x] Live environment context — agent knows current date/time (system clock) + location (`AG2ASSISTANT_LOCATION`), injected per turn via `prompt=[persona, env]` (refreshes; constructor prompts are evaluated once)
...
- [x] Combined runner (`ag2assistant run` — REST + all configured channels, one shared agent) and media/attachments (images/PDFs/audio/video/text → AG2 multimodal inputs; live-verified on Telegram/Slack/Discord)
- [ ] WhatsApp
...
- [x] Skills install to `config.skills_dir` (`~/.ag2assistant/skills`); `LocalRuntime` with blocked-command safety list
...
- [x] AG2 Assistant-bundled skills — first-party `web-research` / `pdf-tools` / `email-drafting` shipped in-package, discoverable on first run via `extra_paths` (no install). Live-verified the agent lists/uses them. (1 test)
- [x] **Google integration (Gmail / Calendar / Drive)** — custom OAuth (`ag2assistant[google]` extra) with desktop consent flow (`ag2assistant google login/logout/status`), token cached + auto-refreshed at `~/.ag2assistant/google_token.json`. Tools auto-appear when signed in: `gmail_search/read/create_draft` (read), `gmail_send` + `calendar_create_event` (write — **HITL-gated**), `calendar_list_events`, `drive_search/read`. Mocked tests (no real OAuth/sends); live verification pending user's Google Cloud OAuth client. (6 tests)
...
AG2 Assistant is an OpenClaw alternative built using AG2's Beta framework. It reimagines OpenClaw's core capabilities (gateway, channels, agent runtime, tools, skills) in Python using AG2 Beta's event-driven architecture instead of Pi agent + TypeScript.

## AG2 Beta Availability (as of May 2026, ag2 0.12.3)

**Major update:** Most of the previously feature-branch-only functionality now ships on main. AG2 Assistant can use AG2's native capabilities for nearly everything.
...
### Since landed on main (as of AG2 0.13.4)

Items this plan once listed as feature-branch-only have since shipped on released
AG2 main, and AG2 Assistant now uses them directly: **Assembly Policies**,
**KnowledgeStore**, **Compaction/Aggregation**, the **Watch/observer** system, and
the **Hub / Network** multi-agent module. AG2 Assistant still does not *depend* on
Hub/Network — it's a single-user assistant — but it's available if a multi-agent
need arises.
...
What AG2 Assistant still owns (not framework concerns):
...
| Roadmap Item | Status | Relevance to AG2 Assistant |
|---|---|---|
...
**Since shipped (AG2 0.13.4):** Assembly Policies, KnowledgeStore, Compaction/Aggregation, the Watch system, and Network/Hub have all landed on released main — AG2 Assistant uses the first four directly. (The `Actor` runtime primitive remains framework-internal.)

## Component Mapping: OpenClaw -> AG2 Assistant

| OpenClaw Component | AG2 Assistant Equivalent | AG2 Beta Status |
|--------------------|-------------------|-----------------|
| Pi Agent runtime | AG2 Beta `Agent` + custom session layer | ✅ Available |
| Gateway (WebSocket) | FastAPI + websockets | Custom (not AG2) |
| Sessions (JSONL) | Custom persistence (SQLite/file) | ✅ KnowledgeStore available (0.13.4) |
...
- **AG2 Assistant** = Application-level, not suitable for an agentic framework

### Phase 1: Core Agent Runtime

| Item | Where | Notes |
|------|-------|-------|
| Project scaffolding (pyproject.toml, src layout) | **AG2 Assistant** | Standard Python project setup, nothing framework-level |
...
| CLI interface (`ag2assistant agent "message"`) | **AG2 Assistant** | App-level CLI (Typer). Not framework concern |
| Config system (Pydantic models for agent/channel/provider settings) | **AG2 Assistant** | App-level config. AG2 has LLM configs (`OpenAIConfig` etc ✅) but overall app config is ours |
...
(`examples/network_gateway_spike.py`) — an AG2 Assistant agent registered on an AG2
...
| REST/WebSocket facade for UI clients | **AG2 Assistant** | Thin layer so non-AG2 UIs (web/desktop) can connect without speaking the Hub protocol. Likely FastAPI in front of the Hub |
| Session management (per user+channel) | **AG2 Assistant + Hub channels** | Map sessions to Hub channels; AG2 Assistant owns user↔channel identity |
| Health/status endpoints | **AG2 Assistant** | App-level ops |

### Phase 3: Channel Integrations

| Item | Where | Notes |
|------|-------|-------|
| Channel adapter interface (protocol/ABC) | **AG2 Assistant** | Defines how messaging platforms plug in. Not framework-level — this is OpenClaw-style app architecture |
| Telegram channel | **AG2 Assistant** | python-telegram-bot integration. App-level adapter |
| Discord channel | **AG2 Assistant** | discord.py integration. App-level adapter |
| Slack channel | **AG2 Assistant** | slack-bolt integration. App-level adapter |
| WhatsApp channel | **AG2 Assistant** | Webhook/API integration. App-level adapter |
| Mention-gating and group message routing | **AG2 Assistant** | App-level message filtering logic before invoking agent |

### Phase 4: Skills & Plugins

| Item | Where | Notes |
|------|-------|-------|
| Skill definition format (SKILL.md equivalent) | **AG2 Assistant** | App-level skill packaging convention. Not framework concern |
...
| Migration path design | **AG2 Assistant** | Design our custom interfaces to align with AG2's unreleased APIs. App-level architecture decision |
...
| Realtime voice | **AG2 ✅ (shipped in AG2 Assistant)** | LiveAgent + Gemini Live. Browser mic→16 kHz PCM over `/api/voice` WS → `RecordedAudioEvent`; `SynthesizedAudioEvent`→24 kHz playback; transcripts as bubbles. Voice agent has a basic tool subset + `ask_assistant` that delegates heavy work to the universal agent. Follow-ups: HITL/permission prompts spoken over voice; barge-in tuning; persist voice transcripts to the session |
...
- CLI smoke tests: `ag2assistant agent "hello"` produces a response
