...
- **Reuses** AG2's `Task` lifecycle/`TaskProgress`/`run_subtasks`/`EventLogWriter` as design reference; AGClaw builds the durable store+tree, runner, iterative HITL intake, the deterministic scheduler (one-shot + recurring), the gateway `TaskService` + REST API, and the Web UI Tasks surface.
